...
_canvas_xs = None      # Size of canvas obj
_canvas_ys = None
...
if _Windows:
    _canvas_t_fonts = ['times new roman', 'lucida console']
else:
    _canvas_t_fonts = ['times', 'lucidasans-24']
...
def begin_graphics(width=640, height=480, color=format_color(0, 0, 0), title=None):
    ...
    _root_window.bind( "<Button-1>", _left_click)
    _root_window.bind( "<Button-2>", _right_click)
    _root_window.bind( "<Button-3>", _right_click)
    _root_window.bind( "<Control-Button-1>", _ctrl_left_click)
    _clear_keys()
...
def _clear_keys(event=None):
    global _keys_down, _got_release, _keys_waiting
    _keys_down = {}
    _keys_waiting = {}
    _got_release = None
...
def keys_pressed(d_o_e=lambda arg: _root_window.dooneevent(arg),
    ...
    if _got_release:
        d_o_e(d_w)
    return _keys_down.keys()

def keys_waiting():
    global _keys_waiting
    keys = _keys_waiting.keys()
    _keys_waiting = {}
    return keys
...
def move_to(obj, x, y=None,
    ...
    horiz = True
    new_coords = []
    current_x, current_y = _canvas.coords(obj)[0:2] # first point
    for coord in  _canvas.coords(obj):
        if horiz:
            inc = x - current_x
        else:
            inc = y - current_y
    ...
    _canvas.coords(obj, *new_coords)
    d_o_e(d_w)

def move_by(obj, x, y=None,
            d_o_e=lambda arg: _root_window.dooneevent(arg),
