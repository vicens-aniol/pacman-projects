...
class Grades:
  ...
  def __init__(self, project_name, questions_and_maxes_list,
    ...
    self.edxOutput = edx_output
    self.gsOutput = gs_output  # GradeScope output
...
""" % (prereq, q, q, prereq))
    ...
    print('------------------')
    print(f'Total: {self.points.total_count()}/{sum(self.maxes.values())}')
...
""")
  ...
  def add_message(self, message, raw=False):
    if not raw:
        ...
        print('*** ' + message)
        if self.mute: util.mute_print()
        # message = cgi.escape(message)  # ERROR: 'cgi' has no attribute 'escape'
