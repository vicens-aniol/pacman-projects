...
def evaluate(generate_solutions, test_root, module_dict, exception_map=ERROR_HINT_MAP,
    ...
    grades = grading.Grades(project_params.PROJECT_NAME, questions, gs_output=gs_output, edx_output=edx_output,
                            mute_output=mute_output)
    if question_to_grade is None:
        for q in question_dicts:
            for pre_req in question_dicts[q].get('depends', '').split():
                grades.add_pre_req(q, pre_req)
...
if __name__ == '__main__':
    ...
    # module_dict = loadModuleDict(module_code_dict)
