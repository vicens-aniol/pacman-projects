...
class GridworldEnvironment(environment.Environment):
    ...
    def get_random_next_state(self, state, action, rand_obj=None):
        ...
        for nextState, prob in successors:
            ...
            if rand < sum_vals:
                reward = self.grid_world.get_reward(state, action, nextState)
...
if __name__ == '__main__':
    ...
    import value_iteration_agents, q_learning_agents
    a = None
    if opts.agent == 'value':
        ...
        qLearnOpts = {'gamma': opts.discount,
                      ...
                      'action_fn': action_fn}
        a = q_learning_agents.QLearningAgent(**qLearnOpts)
        ...
        if not opts.manual and opts.agent in ('value', 'asynchvalue', 'priosweepvalue'):
            if opts.value_steps:
                for i in range(opts.iters):
                    tempAgent = value_iteration_agents.ValueIterationAgent(mdp, opts.discount, i)
