...
def draw_values(gridworld, values, policy, current_state = None, message ='State Values'):
    grid = gridworld.grid
    blank()
    value_list = [values[state] for state in gridworld.get_states()] + [0.0]
...
def draw_q_values(gridworld, q_values, current_state = None, message ='State-Action Q-Values'):
    grid = gridworld.grid
    blank()
    state_cross_actions = [[(state, action) for action in gridworld.get_possible_actions(state)] for state in gridworld.get_states()]
