...
#        self.setupSimulationButtons(win)
    ...
    def setup_alpha_button_and_label(self, win):
        self.alpha_minus = tkinter.Button(win, text="-", command=(lambda: self.increment_alpha(self.dec)))
        ...
        self.alpha_plus = tkinter.Button(win, text="+", command=(lambda: self.increment_alpha(self.inc)))
        self.alpha_plus.grid(row=1, column=5, padx=10)

    def set_up_gamma_button_and_label(self, win):
        self.gamma_minus = tkinter.Button(win, text="-", command=(lambda: self.increment_gamma(self.dec)))
        ...
        self.gamma_plus = tkinter.Button(win, text="+", command=(lambda: self.increment_gamma(self.inc)))
        self.gamma_plus.grid(row=1, column=2, padx=10)

    def setup_epsilon_button_and_label(self, win):
        self.epsilon_minus = tkinter.Button(win, text="-", command=(lambda: self.increment_epsilon(self.dec)))
        ...
        self.epsilon_plus = tkinter.Button(win, text="+", command=(lambda: self.increment_epsilon(self.inc)))
        self.epsilon_plus.grid(row=0, column=5)

    def setup_speed_button_and_label(self, win):
        self.speed_minus = tkinter.Button(win, text="-", command=(lambda: self.increment_speed(.5)))
        self.speed_minus.grid(row=0, column=0)

        self.speed_label = tkinter.Label(win, text='Step Delay: %.5f' % self.tick_time)
        self.speed_label.grid(row=0, column=1)

        self.speed_plus = tkinter.Button(win, text="+", command=(lambda: self.increment_speed(2)))
    ...
    def __init__(self, win):
        ...
        #    self.robot_environment = pendulum.PendulumRobotEnvironment(self.robot)
        else:
            raise Exception("Unknown RobotType")

        # Init Agent
        #simulation_fn = lambda agent: simulation.SimulationEnvironment(self.robot_environment, agent)
